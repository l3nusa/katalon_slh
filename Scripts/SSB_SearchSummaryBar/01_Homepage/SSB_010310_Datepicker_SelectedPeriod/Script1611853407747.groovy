...
while(date > new Date()){
	def classSelected = WebUI.getAttribute(CustomKeywords.'customPackage.ssb.getDatePickerObjectByDate'(date), 'class')
	
	if (!(classSelected.contains('CalendarDay__selected_span'))) {
		throw new Exception('Day isnot greyed out between checkin/checkout')
	}
